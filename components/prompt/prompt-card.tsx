...
import { MoreHorizontal, Image, ArrowRight, X, Clock, ExternalLink } from "lucide-react";
...
// Logical workflow progression map for prompts
...
export const PromptCard: React.FC<PromptCardProps> = ({
  ...
  return (
    ...
    >
      <Card
        className={cn(
          "hover:border-primary/45 transition-all duration-200 bg-card border border-l-[3px] border-l-primary/60 shadow-sm hover:shadow-md relative group"
        )}
      >
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 ml-auto overflow-hidden">
              ...
              {relativeTimeStr && moduleName && (
                <span className="text-muted-foreground/30 text-[10px] shrink-0 select-none">•</span>
              )}
              {moduleName && (
                <span
                  className="text-[10px] text-muted-foreground bg-muted border border-border px-1.5 py-0.5 rounded font-medium truncate max-w-[120px]"
                  title={moduleName}
                >
                  {moduleName}
                </span>
          ...
          <div className="flex items-center justify-between text-[10px] text-muted-foreground pt-3 border-t border-border/50 h-8">
            <div className="flex items-center gap-1">
              {screenshotUrl && (
                <span
                  className={cn(
                    "inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-md transition-all cursor-pointer font-semibold shadow-sm select-none border",
                    isDirectImage
