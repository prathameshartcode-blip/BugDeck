        ...
        <DialogContent onClose={() => setIsModulesOpen(false)} className="max-w-md">
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
              {modules.filter(m => m.project_id === projectId).length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-4">No modules yet. Add one below.</p>
              )}
              {modules.filter(m => m.project_id === projectId).map((mod) => (
                <div key={mod.id} className="flex items-center justify-between gap-2 px-3 py-2 rounded-lg border border-border bg-muted/20 text-xs">
                  ...
                  <button
                    onClick={async () => {
                      if (confirm(`Delete module "${mod.name}"? Bugs in this module will lose their module assignment.`)) {
                        await deleteModule(mod.id, projectId);
                      }
                    }}
            ...
            <div className="pt-2 border-t border-border space-y-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onKeyDown={async (e) => {
                    if (e.key === "Enter" && addingModuleName.trim() && !addingModule) {
